...
async function createUser(email, nombre, apellido, password) {
  const res = await fetch(EDGE_URL, {
    ...
    body: JSON.stringify({ email, nombre, apellido, password, accion: 'crear', nivel: 'ADMIN' })
  });
  const data = await res.json();
  console.log(`Result for ${email}:`, data);
}

createUser('admin.prueba4@gmail.com', 'Admin', 'Prueba4', 'bajardepeso').catch(console.error);
