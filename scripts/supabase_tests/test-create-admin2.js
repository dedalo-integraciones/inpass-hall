...
async function createUser(email, nombre, apellido, password) {
  const res = await fetch(EDGE_URL, {
    ...
    body: JSON.stringify({ email, nombre, apellido, password, accion: 'crear_admin', nivel: 'ADMIN' })
  });
  const data = await res.json();
  console.log(`Result for ${email}:`, data);
}

createUser('admin.prueba5@gmail.com', 'Admin', 'Prueba5', 'bajardepeso').catch(console.error);
